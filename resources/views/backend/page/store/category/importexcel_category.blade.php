                ...
                <ul class="nav nav-tabs nav-tabs-line" id="lineTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" id="home-line-tab" data-bs-toggle="tab" href="#home" role="tab"
                           aria-controls="home" aria-selected="true">Import danh mục</a>
                    </li>
                </ul>
                                ...
                                <div class="row">
                                    <div class="col-12 col-lg-6">
                                        <div class="alert alert-info">
                                            Tải file mẫu
                                            <a href="{{ asset('excel_templates/AUCA_Import_Category.xlsx') }}"
                                               download="">AUCA_Import_Category.xlsx</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                ...
                <div class="tab-content mt-3" id="result-content" style="display: none">
                    <div class="tab-pane fade show active" id="home" role="tabpanel"
                        ...
                        <div class="card-body p-0 pt-2">
                            <div class="row">
                                <div class="col-12 col-lg-6">
                                    <div class="alert alert-primary" role="alert" id="res-total-count"  style="display: none">
                                        Tổng số xử lý: <b id="total_record"></b>
                                    </div>
                                    <div class="alert alert-success" role="alert" id="res-total-success" style="display: none">>
                ...
                $.ajax({
                            ...
                            if (response.data.fail_data.length === 0) {
                                ...
                                response.data.fail_data.forEach(function(item) {
                                    ...
                                    if(Array.isArray(item)){
                                        ...
                                        errorParagraph.append(item.errors + '<br>');
                                    }
                    ...
                    error: function (error) {
                        ...
                        if (error.responseJSON && error.responseJSON.message) {
                            ...
                            if (error.responseJSON.data && error.responseJSON.data.fail_data) {
                                var data = error.responseJSON.data;
                                if (data.fail_data.length === 0) {
                                    $('#res-error').hide();
                                } else {
                                    $('#result-content').show();
                                    $('#res-error').show();
                                    var resErrorDiv = $('#res-error');
                                    resErrorDiv.empty();
                                    data.fail_data.forEach(function(item) {
                                        var errorParagraph = $('<p></p>');
                                        errorParagraph.append('Dòng ' + item.row_index + ': ');
                                        if(Array.isArray(item)){
                                            item.errors.forEach(function(error) {
